...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const apolloModel = {
      ...
      api: {
        id: "apollo-test",
        url: "https://api.github.com/i-luv-pho/apollov2",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, apolloModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.apollo?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.apollo?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.apollo?.itemId).toBe("msg_apollo")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.apollo?.itemId).toBe("msg_apollo_part")
