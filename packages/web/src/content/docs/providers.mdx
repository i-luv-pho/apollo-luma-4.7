---
title: Providers
description: Using any LLM provider in Apollo.
---

import config from "../../../config.mjs"
export const console = config.console

Apollo uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Apollo config.
...
in `~/.local/share/apollo/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Apollo
config.
...
Apollo Zen is a list of models provided by the Apollo team that have been
tested and verified to work well with Apollo. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Apollo Zen.
:::

1. Run the `/connect` command in the TUI, select apollo, and head to [github.com/i-luv-pho/apollov2/auth](https://github.com/i-luv-pho/apollov2/auth).
...
It works like any other provider in Apollo and is completely optional to use.
...
To use Amazon Bedrock with Apollo:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running apollo:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY apollo

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile apollo

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX apollo
   ```
   ...
   For project-specific or persistent configuration, use `apollo.json`:

   ```json title="apollo.json"
   {
     "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
     "provider": {
       "amazon-bedrock": {
...
You can sign up for an Anthropic subscription or use an API key.
...
2. Here you can select the **Subscription** option and it'll open your browser
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for apollo to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX apollo
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX apollo
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your apollo config.
...
```bash
GITLAB_INSTANCE_URL=https://gitlab.company.com GITLAB_TOKEN=glpat-xxxxxxxxxxxxxxxxxxxx apollo
```
...
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "plugin": ["@gitlab/apollo-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with apollo:
...
To use Google Vertex AI with Apollo:
...
2. Set the required environment variables:
   ...
   Set them while running apollo.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id apollo
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through apollo, you can always configure it yourself.
...
```jsonc title="~/.config/apollo/apollo.jsonc"
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [apollo-helicone-session](https://github.com/H2Shami/apollo-helicone-session) plugin to automatically log each Apollo conversation as a session in Helicone.

```bash
npm install -g apollo-helicone-session
```
...
```json title="apollo.json"
{
  "plugin": ["apollo-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Apollo conversation listed as a separate session.
...
You can configure apollo to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="apollo.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure apollo to use local models through LM Studio.

```json title="apollo.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "provider": {
    "lmstudio": {
...
You can configure apollo to use local models through Ollama.

```json title="apollo.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Apollo:
...
3. Copy the API key for use in Apollo.
...
6. **Important**: Before using cloud models in Apollo, you must pull the model information locally:
...
Apollo Zen is a list of tested and verified models provided by the Apollo team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Apollo Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Apollo Zen**.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your apollo config.
...
5. You can also customize them through your apollo config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' apollo
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group apollo
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Apollo:
...
You can also customize models through your apollo config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with apollo. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in apollo.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `apollo.json` file in your project directory:

   ```json title="apollo.json" ""myprovider"" {5-15}
   {
     "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
     "provider": {
       "myprovider": {
...
```json title="apollo.json" {9,11,17-20}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Apollo to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `apollo auth list` to see if the credentials
...
2. For custom providers, check the apollo config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your apollo config.
