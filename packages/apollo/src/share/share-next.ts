...
import type * as SDK from "@apollo-ai/sdk/v2"

export namespace ShareNext {
  const log = Log.create({ service: "share-next" })

  async function url() {
    return Config.get().then((x) => x.enterprise?.url ?? "https://github.com/i-luv-pho/apollov2")
  }
  ...
  async function sync(sessionID: string, data: Data[]) {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          log.error("sync failed", { sessionID, status: response.status })
        }
      } catch (error) {
        log.error("sync error", { sessionID, error: error instanceof Error ? error.message : String(error) })
      }
    }, 1000)
