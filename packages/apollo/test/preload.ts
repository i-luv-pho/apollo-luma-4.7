...
const dir = path.join(os.tmpdir(), "apollo-test-data-" + process.pid)
await fs.mkdir(dir, { recursive: true })
afterAll(() => {
  fsSync.rmSync(dir, { recursive: true, force: true })
})
// Set test home directory to isolate tests from user's actual home directory
// This prevents tests from picking up real user configs/skills from ~/.apollo/skills
const testHome = path.join(dir, "home")
await fs.mkdir(testHome, { recursive: true })
process.env["APOLLO_TEST_HOME"] = testHome
...
const cacheDir = path.join(dir, "cache", "apollo")
...
process.env["APOLLO_DISABLE_MODELS_FETCH"] = "true"
