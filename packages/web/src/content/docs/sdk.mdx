---
title: SDK
description: Type-safe JS client for apollo server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The apollo JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control apollo programmatically.
...
const { client } = await createApollo()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `apollo.json`, but you can override or add configuration inline:
...
console.log(`Server running at ${apollo.server.url}`)
...
If you already have a running instance of apollo, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@apollo-ai/sdk"
```
