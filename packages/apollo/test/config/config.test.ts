  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        ...
        JSON.stringify({
          $schema: "https://github.com/i-luv-pho/apollov2/config.json",
          model: "test/model",
          username: "testuser",
        }),
      )
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "apollo.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "apollo.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .apollo directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const apolloDir = path.join(dir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })
      const agentDir = path.join(apolloDir, "agent")
...
test("loads agents from .apollo/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const apolloDir = path.join(dir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })

      const agentsDir = path.join(apolloDir, "agents")
...
test("loads commands from .apollo/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const apolloDir = path.join(dir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })

      const commandDir = path.join(apolloDir, "command")
...
test("loads commands from .apollo/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const apolloDir = path.join(dir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })

      const commandsDir = path.join(apolloDir, "commands")
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "apollo.json"),
        JSON.stringify({ $schema: "https://github.com/i-luv-pho/apollov2/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "apollo.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .apollo config
      const projectDir = path.join(dir, "project")
      const apolloDir = path.join(projectDir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })
      ...
      // Local .apollo config with different plugins
      await Bun.write(
        path.join(apolloDir, "apollo.json"),
        JSON.stringify({
          $schema: "https://github.com/i-luv-pho/apollov2/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .apollo config with some overlapping plugins
      await Bun.write(
        path.join(apolloDir, "apollo.json"),
        JSON.stringify({
          $schema: "https://github.com/i-luv-pho/apollov2/config.json",
...
test("local .apollo config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .apollo directory config enables it
      const apolloDir = path.join(dir, ".apollo")
      await fs.mkdir(apolloDir, { recursive: true })
      await Bun.write(
        path.join(apolloDir, "apollo.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/apollo")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/apollo")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-apollo@2.4.3")).toBe("oh-my-apollo")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@apollo/plugin@2.0.0")).toBe("@apollo/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-apollo@2.4.3", "file:///project/.apollo/plugin/oh-my-apollo.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.apollo/plugin/oh-my-apollo.js")
  })
  ...
  test("local plugin directory overrides global apollo.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const apolloDir = path.join(projectDir, ".apollo")
        const pluginDir = path.join(apolloDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
