...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run apollo",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for apollo session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found apollo session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`apollo import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting apollo...")
        UI.println()

        // Launch apollo TUI with session ID if available
        const { spawn } = await import("child_process")
        const apolloArgs = sessionId ? ["-s", sessionId] : []
        const apolloProcess = spawn("apollo", apolloArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          apolloProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`apollo exited with code ${code}`))
          })
          apolloProcess.on("error", reject)
        })
      },
    })
  },
