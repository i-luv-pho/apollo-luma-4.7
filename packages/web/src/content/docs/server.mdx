---
title: Server
description: Interact with apollo server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `apollo serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an apollo client can use.
...
```bash
apollo serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
apollo serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `APOLLO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `apollo`, or set `APOLLO_SERVER_USERNAME` to override it. This applies to both `apollo serve` and `apollo web`.

```bash
APOLLO_SERVER_PASSWORD=your-password apollo serve
```
...
When you run `apollo` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the apollo server to interact with apollo programmatically.
:::

This architecture lets apollo support multiple clients and allows you to interact with apollo programmatically.

You can run `apollo serve` to start a standalone server. If you have the
apollo TUI running, `apollo serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Apollo [IDE](/docs/ide) plugins.
...
The apollo server exposes the following APIs.
