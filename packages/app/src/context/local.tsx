...
import type { FileContent, FileNode, Model, Provider, File as FileStatus } from "@apollo-ai/sdk/v2"
import { createSimpleContext } from "@apollo-ai/ui/context"
  ...
  init: () => {
    ...
    const model = (() => {
      ...
      const list = createMemo(() =>
        available()
          // Sort Apollo first, then alphabetically
          .sort((a, b) => {
            if (a.provider.id === "apollo" && b.provider.id !== "apollo") return -1
            if (a.provider.id !== "apollo" && b.provider.id === "apollo") return 1
            return a.provider.name.localeCompare(b.provider.name)
          })
          .map((m) => ({
            ...m,
            // White-label: Apollo models display as Luna 4.7
            name: m.provider.id === "apollo" ? "Luna 4.7" : m.name.replace("(latest)", "").trim(),
