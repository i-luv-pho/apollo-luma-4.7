...
With Apollo you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Apollo uses our own `apollo` theme.
...
Apollo comes with several built-in themes.
...
- Want Apollo to match their terminal's appearance
...
Apollo supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/apollo/themes/*.json` or `$XDG_CONFIG_HOME/apollo/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.apollo/themes/*.json`
4. **Current working directory** - Defined in `./.apollo/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/apollo/themes
vim ~/.config/apollo/themes/my-theme.json
```

And for project-specific themes.

```bash no-frame
mkdir -p .apollo/themes
vim .apollo/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/i-luv-pho/apollov2/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
