...
const FORK_REPO = "i-luv-pho/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "apollo"
...
async function main() {
  ...
  // Delete any existing branches for apollo updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating Apollo extension to v${cleanVersion}"`
