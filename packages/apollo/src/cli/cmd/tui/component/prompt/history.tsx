...
import type { AgentPart, FilePart, TextPart } from "@apollo-ai/sdk/v2"
...
export const { use: usePromptHistory, provider: PromptHistoryProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      search(query: string, startIndex?: number): { entry: PromptInfo; index: number } | undefined {
        if (!query || !store.history.length) return undefined
        const lowerQuery = query.toLowerCase()
        const start = startIndex !== undefined ? startIndex - 1 : store.history.length - 1
        for (let i = start; i >= 0; i--) {
          const entry = store.history[i]
          if (entry.input.toLowerCase().includes(lowerQuery)) {
