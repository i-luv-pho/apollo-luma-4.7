...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for apollo.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal apollo sessions
...
apollo acp --cwd /path/to/project
```
...
- **Terminal Integration**: Full terminal support via apollo's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | apollo acp
```
...
ACP sessions map cleanly to apollo's internal session model:
