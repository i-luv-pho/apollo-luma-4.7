---
title: Custom Tools
description: Create tools the LLM can call in apollo.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside apollo's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.apollo/tools/` directory of your project.
- Or globally, by placing them in `~/.config/apollo/tools/`.
...
```ts title=".apollo/tools/database.ts" {1}
...
```ts title=".apollo/tools/project.ts" {8}
import { tool } from "@apollo-ai/plugin"
...
```python title=".apollo/tools/add.py"
import sys
...
```ts title=".apollo/tools/python-add.ts" {10}
import { tool } from "@apollo-ai/plugin"
...
export default tool({
  ...
  async execute(args) {
    const result = await Bun.$`python3 .apollo/tools/add.py ${args.a} ${args.b}`.text()
    return result.trim()
  },
})
```
