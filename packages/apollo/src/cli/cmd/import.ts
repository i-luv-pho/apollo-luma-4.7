...
export const ImportCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs.positional("file", {
      describe: "path to JSON file or github.com/i-luv-pho share URL",
      type: "string",
      demandOption: true,
    })
  },
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      ...
      if (isUrl) {
        ...
        if (!urlMatch) {
          process.stdout.write(`Invalid URL format. Expected: https://github.com/i-luv-pho/apollov2/share/<slug>`)
          process.stdout.write(EOL)
          return
        }

        const slug = urlMatch[1]
        const response = await fetch(`https://api.github.com/i-luv-pho/share/${slug}`)
