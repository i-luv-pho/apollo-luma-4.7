...
Agents are autonomous AI entities that can be configured for specific tasks and workflows. They allow you to create powerful, focused tools with custom prompts, models, and tool access.
...
There are two types of agents in Apollo; primary agents and subagents.

---

### Primary agents

Primary agents are the main agents you interact with directly. You can cycle through them using the **Tab** key, or your configured `switch_agent` keybind. These agents handle your main conversation and can access all configured tools.

:::tip
You can use the **Tab** key to switch between primary agents during a session.
:::

Apollo comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Apollo comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Apollo comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `apollo.json` config file:
...
- Global: `~/.config/apollo/agents/`
- Per-project: `.apollo/agents/`

```markdown title="~/.config/apollo/agents/review.md"
---
...
If no temperature is specified, Apollo uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Apollo config and the project specific config.
...
The model ID in your Apollo config uses the format `provider/model-id`. For example, if you're using [Apollo Zen](/docs/zen), you would use `apollo/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="apollo.json" {3-6,9-12}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "tools": {
    "write": true,
...
```json title="apollo.json" {3-5,8-10}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `apollo models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/metamorphis-labs/apollo).
:::
...
```markdown title="~/.config/apollo/agents/docs-writer.md"
---
...
```markdown title="~/.config/apollo/agents/security-auditor.md"
---
