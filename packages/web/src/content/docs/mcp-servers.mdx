...
You can add external tools to Apollo using the _Model Context Protocol_, or MCP. Apollo supports both local and remote servers.
...
You can define MCP servers in your [Apollo Config](https://github.com/i-luv-pho/apollov2/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="apollo.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/apollo` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Apollo automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Apollo will:
...
If the server requires authentication, Apollo will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `apollo mcp auth <server-name>`.
...
```bash
apollo mcp auth my-oauth-server
```
...
```bash
apollo mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Apollo will store the tokens securely in `~/.local/share/apollo/mcp-auth.json`.
...
apollo mcp debug my-oauth-server
```
...
Your MCPs are available as tools in Apollo, alongside built-in tools. So you can manage them through the Apollo config like any other tool.
...
```json title="apollo.json" {11, 14-18}
{
  "$schema": "https://github.com/i-luv-pho/apollov2/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
apollo mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect Apollo to your Sentry account.
