...
export namespace MCP {
        ...
        authProvider = new McpOAuthProvider(
          ...
          },
          {
            onRedirect: async (url) => {
              // Security: Don't log full URL - it may contain sensitive state/code parameters
              log.info("oauth redirect requested", { key, host: url.host })
        ...
        try {
          ...
          if (error instanceof UnauthorizedError) {
            ...
            if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
              ...
              Bus.publish(TuiEvent.ToastShow, {
                title: "MCP Authentication Required",
                message: `Server "${key}" requires authentication. Run: apollo mcp auth ${key}`,
                variant: "warning",
                duration: 8000,
              }).catch((e) => log.debug("failed to show toast", { error: e }))
            }
  ...
  export async function startAuth(mcpName: string): Promise<{ authorizationUrl: string }> {
    ...
    const oauthState = Array.from(crypto.getRandomValues(new Uint8Array(32)))
      ...
      .join("")
    await McpAuth.updateOAuthState(mcpName, oauthState, mcpConfig.url)
  ...
  export async function authenticate(mcpName: string): Promise<Status> {
    ...
    log.info("opening browser for oauth", { mcpName, hasAuthUrl: !!authorizationUrl })
    try {
