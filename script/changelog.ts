...
import { $ } from "bun"
import { createApollo } from "@apollo-ai/sdk"
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  // Handle first release - no commits to compare
  if (from === "0.0.0") return []
  ...
  let compare: string
  try {
    compare = await $`gh api "/repos/i-luv-pho/apollov2/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  } catch {
    // If compare fails (e.g., tag doesn't exist), return empty array
    return []
  }
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/apollo packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/apollo/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/apollo/")) areas.add("core")
...
async function summarizeCommit(apollo: Awaited<ReturnType<typeof createApollo>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await apollo.client.session.create()
  const result = await apollo.client.session
    .prompt({
      path: { id: session.data!.id },
      body: {
        model: { providerID: "apollo", modelID: "claude-haiku-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], apollo: Awaited<ReturnType<typeof createApollo>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(apollo, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/i-luv-pho/apollov2/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const apollo = await createApollo({ port: 5044 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, apollo)
    notes.push(...lines)
