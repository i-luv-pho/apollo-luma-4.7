...
export namespace Storage {
  ...
  // Validate storage keys to prevent directory traversal
  function validateKey(key: string[]): void {
    for (const part of key) {
      if (part.includes("..") || part.includes("/") || part.includes("\\") || part.includes("\0")) {
        throw new Error(`Invalid storage key: ${part}`)
      }
    }
  }
  ...
  const state = lazy(async () => {
    ...
    for (let index = migration; index < MIGRATIONS.length; index++) {
      ...
      try {
        ...
        // Don't increment migration index on failure - will retry on next startup
        throw new Error(`Storage migration ${index} failed: ${error}`)
      }
    }
    return {
      dir,
