...
:::caution
Modes are now configured through the `agent` option in the apollo config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in apollo allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the apollo config.
...
apollo comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.apollo/plans/*.md` to detail the plan itself
...
Configure modes in your `apollo.json` config file:
...
- Global: `~/.config/apollo/modes/`
- Project: `.apollo/modes/`

```markdown title="~/.config/apollo/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
both the global apollo config and the project specific config.
...
Create mode files in `.apollo/modes/` for project-specific modes or `~/.config/apollo/modes/` for global modes:

```markdown title=".apollo/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/apollo/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
