...
import { createApolloClient, type ApolloClient } from "@apollo-ai/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run apollo with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running apollo server (e.g., http://localhost:4096)",
      })
      .option("port", {
        type: "number",
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: ApolloClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createApolloClient({ baseUrl: args.attach })
      ...
      if (cfgResult.data && (cfgResult.data.share === "auto" || Flag.APOLLO_AUTO_SHARE || args.share)) {
