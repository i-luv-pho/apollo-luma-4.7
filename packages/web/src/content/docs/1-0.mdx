...
description: What's new in Apollo 1.0.
---

Apollo 1.0 is a complete rewrite of the TUI.

We moved from the go+bubbletea based TUI which had performance and capability issues to an in-house framework (ApolloTUI) written in zig+solidjs.

The new TUI works like the old one since it connects to the same apollo server.
...
version. However some older versions of Apollo always grab latest.

To upgrade manually, run

```bash
$ apollo upgrade 1.0.0
```

To downgrade back to 0.x, run

```bash
$ apollo upgrade 0.15.31
```
