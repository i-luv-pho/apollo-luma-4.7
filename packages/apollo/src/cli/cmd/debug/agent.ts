...
function parseToolParams(input?: string) {
  ...
  // Security: Only accept valid JSON to prevent code injection
  // Previously used new Function() which allowed arbitrary code execution
  let parsed: unknown
  try {
    parsed = JSON.parse(trimmed)
  } catch (jsonError) {
    throw new Error(
      `Failed to parse --params. Must be valid JSON (e.g., '{"key": "value"}'). Error: ${jsonError}`,
    )
  }

  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("Tool params must be a JSON object (e.g., '{\"key\": \"value\"}').")
  }
