...
import type { ApolloClient, SessionMessageResponse } from "@apollo-ai/sdk/v2"
import { applyPatch } from "diff"

export namespace ACP {
  const log = Log.create({ service: "acp-agent" })

  export async function init({ sdk: _sdk }: { sdk: ApolloClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `apollo auth login` in the terminal",
        name: "Login with apollo",
        id: "apollo-login",
      }
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const apolloProvider = providers.find((p) => p.id === "apollo")
    if (apolloProvider) {
      if (apolloProvider.models["big-pickle"]) {
        return { providerID: "apollo", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(apolloProvider.models))
      if (best) {
        return {
